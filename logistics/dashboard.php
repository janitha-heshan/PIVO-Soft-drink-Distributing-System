...
<body>
  ...
  <header class="topbar">
    ...
    <nav class="dash-nav">
      ...
      <button class="avatar" style="background:#f57c00;">L</button>
    </nav>
  </header>
  ...
  <main class="container">
    ...
    <div class="row" style="display:flex; gap: 24px;">
                ...
                <?php if(count($orders)>0): ?>
                    ...
                    <div class="product-card" style="display:flex; justify-content:space-between; align-items:center;">
                        <div>
                            ...
                            <p style="text-transform:uppercase; font-size:11px; font-weight:bold; color:#666; margin-bottom:4px;"><?php echo htmlspecialchars($ord['shop_name']); ?></p>
                            <p style="font-size:13px; color:#444;">📍 <?php echo htmlspecialchars($ord['address']); ?></p>
                            <p style="font-size:13px; color:#444;">📞 <?php echo htmlspecialchars($ord['contact_number']); ?></p>
                        </div>
                        <div style="text-align:right;">
                            <span class="pill" style="margin-bottom:8px; display:inline-block; font-size:11px;"><?php echo $ord['delivery_status']; ?></span>
                            <br>
                            <button onclick="markDelivered(<?php echo $ord['order_id']; ?>)" class="primary" style="padding: 8px 12px; font-size:13px;">Verify Delivery</button>
                        </div>
                    </div>
                    <?php endforeach; ?>
                <?php else: ?>
        ...
        <!-- Map Placeholder -->
        <div style="flex: 1; background:#e0e0e0; border-radius:12px; min-height:400px; display:flex; align-items:center; justify-content:center; color:#777; position:relative; overflow:hidden;">
            <?php if(!empty($apiKey)): ?>
                <iframe
                  width="100%"
                  height="100%"
